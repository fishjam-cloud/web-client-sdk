...
import { mergeWithDefaultBandwidthLimits } from "./bandwidth";

it("defaults every limit to the cap", () => {
  expect(mergeWithDefaultBandwidthLimits()).toEqual({
...
it("keeps user values and fills the missing simulcast layers with the cap", () => {
  const merged = mergeWithDefaultBandwidthLimits({ singleStream: 800, simulcast: { [Variant.VARIANT_HIGH]: 1000 } });
