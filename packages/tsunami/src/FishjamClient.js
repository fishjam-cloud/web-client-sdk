...
export class FishjamClient extends EventEmitter {
    ...
    constructor(config) {
        super();
        const { deviceManager, videoConstraints, audioConstraints, bandwidthLimits, videoStreamConfig, audioStreamConfig, signallingClient, ...createConfig } = config ?? {};
    ...
    requireDevices() {
        ...
        if (!this.deviceOrchestrator)
            throw new DeviceManagerMissingError();
        return this.deviceOrchestrator;
    }
    createTrackPublisher() {
        ...
        return {
            addTrack: (track, metadata, simulcastConfig, maxBandwidth) => this.addTrack(asSignallingTrack(track), metadata, simulcastConfig, maxBandwidth),
            ...
            resolveRemoteTrackId: (remoteOrLocalTrackId) => {
                ...
                if (!tracks)
                    return null;
                if (tracks.get(remoteOrLocalTrackId))
                    return remoteOrLocalTrackId;
    ...
    emit(event, ...args) {
        if (this.tsClient)
            return this.tsClient.emit(event, ...args);
    ...
    getBandwidthEstimation() {
        this.resources.assertActive();
        if (!this.tsClient)
            throw new Error("WebRTC is not initialized");
    ...
    bindSessionStateEvents() {
        ...
        const reconnectionErrorIfReconnecting = () => this.store.getState().reconnectionStatus === "reconnecting" ? { reconnectionStatus: "error" } : {};
        ...
        this.on("reconnected", () => this.store.update({ peerStatus: "connected", reconnectionStatus: "idle", ...participants() }));
