...
export class CustomSourceController {
    ...
    constructor(deps) {
        ...
        this.sessionCleanups = [
            deps.publisher.onJoined(() => {
                void this.enqueue(() => this.publishPendingSources()).catch((error) => this.deps.logger.error("Failed to publish custom sources", error));
            }),
            deps.publisher.onDisconnected(() => {
                ...
                if (!hasPublishedSources)
                    return;
                this.sources = Object.fromEntries(Object.entries(this.sources).map(([id, source]) => [id, { stream: source.stream }]));
                this.notify();
            }),
        ];
    }
    ...
    async publishPendingSources() {
        ...
        for (const [, started] of orphans) {
            if (started.trackIds)
                await this.removeTracks(started.trackIds);
        }
    }
    async publishSource(source) {
        ...
        const promises = [];
        if (video)
            promises.push(this.addTrack(video, { type: "customVideo", displayName, paused: false }));
        if (audio)
            promises.push(this.addTrack(audio, { type: "customAudio", displayName, paused: false }));
    ...
    async removeTracks({ videoId, audioId }) {
        const promises = [];
        if (videoId)
            promises.push(this.deps.publisher.removeTrack(videoId));
        if (audioId)
            promises.push(this.deps.publisher.removeTrack(audioId));
