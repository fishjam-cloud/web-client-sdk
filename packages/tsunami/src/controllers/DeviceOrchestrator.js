...
export class DeviceOrchestrator {
    ...
    async initializeDevices(settings) {
        ...
        const constraints = {
            video: settings?.enableVideo !== false && prepareConstraints(lastUsed.video?.deviceId, this.deps.videoConstraints),
            audio: settings?.enableAudio !== false && prepareConstraints(lastUsed.audio?.deviceId, this.deps.audioConstraints),
        };
        const initialize = async () => {
            let media = await getAvailableMedia(this.deps.deviceManager, constraints);
            await this.refreshDeviceList();
            if (media.stream) {
                media = await correctDevicesOnSafari(this.deps.deviceManager, media.stream, media.errors, this.deviceList, constraints, lastUsed);
            }
            ...
            if (videoDevice)
                this.camera.setSelectedDevice(videoDevice);
            if (audioDevice)
                this.microphone.setSelectedDevice(audioDevice);
        ...
        const initializationPromise = initialize().then((result) => {
    ...
    persistLastDevice(type, device) {
        void Promise.resolve(this.deps.deviceManager.persistence?.saveLastDevice(type, device)).catch((error) => this.deps.logger.warn({ name: "Failed to persist last device", error }));
    }
    syncStore() {
