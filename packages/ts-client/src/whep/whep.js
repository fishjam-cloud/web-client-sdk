...
export class WHEPClient extends EventTarget {
  ...
  async view(pc, url, token) {
    ...
    pc.onicecandidate = (event) => {
      ...
      if (!this.iceTrickeTimeout && !this.restartIce) this.iceTrickeTimeout = setTimeout(() => this.patch(), 0);
    };
    //Create SDP offer
    ...
    if (links.hasOwnProperty(Extensions.Core.Layer)) this.layerUrl = new URL(links[Extensions.Core.Layer][0].url, url);
      ...
      for (const server of links['ice-server']) {
        try {
          ...
          for (const [key, value] of Object.entries(server.params)) {
            ...
            const camelCase = key.replace(/([-_][a-z])/gi, ($1) => $1.toUpperCase().replace('-', '').replace('_', ''));
            //Unquote value and set them
    ...
    if (!this.iceTrickeTimeout) this.iceTrickeTimeout = setTimeout(() => this.patch(), 0);
