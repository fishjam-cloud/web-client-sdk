...
describe("camera", () => {
  ...
  it("startCamera exposes the track in state without publishing when not connected", async ({
    media,
    service,
    signalling,
  }) => {
  ...
  it("toggleCamera while connected publishes one video track with camera metadata", async ({
    media,
    service,
    signalling,
  }) => {
  ...
  it("toggleCamera off pauses the published track and does not remove it", async ({ media, service, signalling }) => {
    ...
    expect(signalling.updateTrackMetadata).toHaveBeenCalledWith(
      publishedTrackId,
      expect.objectContaining({ type: "camera", paused: true }),
    );
  });
