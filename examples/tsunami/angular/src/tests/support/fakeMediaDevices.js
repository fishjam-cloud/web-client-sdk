 ...
 */
const resolveTrack = (installed, kind, constraint) => {
    ...
    if (requested?.mandatory) {
        ...
        if (!device)
            throw makeOverconstrainedError();
    }
    else {
...
export const installFakeMediaDevices = () => {
    let userMediaTemplate = new FakeMediaStream();
    let userMediaError = null;
    let displayMediaStreamFactory = () => new FakeMediaStream();
    let enumerated = [];
    ...
    const installedDevices = () => {
        const fromTemplate = userMediaTemplate.getTracks().map((track) => ({
            kind: track.kind,
        ...
        const fromEnumerated = enumerated
            ...
            .filter((device) => !fromTemplate.some((template) => template.kind === device.kind && template.deviceId === device.deviceId));
        return [...fromTemplate, ...fromEnumerated];
    };
    const getUserMedia = vi.fn(async (constraints = {}) => {
        if (userMediaError)
            throw makeError(userMediaError);
