...
const installedDevices = [
  { deviceId: "camera-1", kind: "videoinput" as const, label: "Camera 1" },
  { deviceId: "microphone-1", kind: "audioinput" as const, label: "Microphone 1" },
];
...
describe("initializeDevices", () => {
  ...
  it("deduplicates: a second call reports already_initialized without touching the platform again", async ({
    media,
    service,
  }) => {
    media.setUserMediaStream(fullStream());
