import type { TracksMiddleware } from "@fishjam-cloud/tsunami";
...
describe("screen share", () => {
  ...
  it("startScreenShare while connected publishes screen share video and audio tracks", async ({
    media,
    service,
    signalling,
  }) => {
    ...
    const publishedTypes = signalling.addTrack.mock.calls.map((call) => (call[1] as { type: string }).type);
  ...
  it("setScreenShareTracksMiddleware persists in state and replaces the published tracks", async ({
    media,
    service,
    signalling,
  }) => {
    ...
    const middleware: TracksMiddleware = (videoTrack, audioTrack) => ({
