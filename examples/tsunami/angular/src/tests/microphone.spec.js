...
describe("microphone", () => {
    ...
    it("toggleMicrophone while connected publishes one audio track with microphone metadata", async ({ media, service, signalling, }) => {
    ...
    it("toggleMicrophoneMute flips isEnabled and pauses the published track via replaceTrack(null)", async ({ media, service, signalling, }) => {
        media.setUserMediaStream(audioStream());
        await connectAndJoin(service, signalling);
        ...
        expect(signalling.updateTrackMetadata).toHaveBeenLastCalledWith(publishedTrackId, expect.objectContaining({ type: "microphone", paused: true }));
    ...
    it("unmuting flips isEnabled back and resumes the published track", async ({ media, service, signalling }) => {
        ...
        expect(signalling.updateTrackMetadata).toHaveBeenLastCalledWith(publishedTrackId, expect.objectContaining({ type: "microphone", paused: false }));
    });
});
